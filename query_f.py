...
def describe_team(team,descriptive_stats):
...
def handle_datasets(stats,skills,search_req,bht):
    stats = stats.rename(columns={'name': 'hero_name'})
    search_dt = skills.merge(stats, on='hero_name')
    if bht=='on':
        search_dt=search_dt[search_dt['hero_name']!='Bounty Hunter']
...
def optimize_skills(search_req,result_b,pysqldf):
    ...
    X=', '.join("{}".format(item) for item in x if item!='')
    ...
    WHERE r.rn <= 4
    ORDER BY r.hero_name, {} DESC""".format(X,X,X)
    df3=pysqldf(q4)
